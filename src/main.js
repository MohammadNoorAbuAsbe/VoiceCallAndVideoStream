  ...
  relay
    ...
    .on('audio',     (bytes) => {
      ...
      const chain = (activeCall._recvChain || Promise.resolve())
        ...
        .then((opusBytes) => audio.playOpusFrame(new Uint8Array(opusBytes)))
...
// ─── Audio frame wiring ───────────────────────────────────────────────────────────
// Each Opus frame is encrypted individually and sent as its own WebSocket binary
// message. No batching needed — Opus packets at 20 kbps are tiny (~50 bytes).
let _starveStart = 0;

// Encrypt each captured Opus frame end-to-end, then send it. Frames are chained
// so ciphertext is emitted in capture order. No plaintext is ever sent: if the
// secure session isn't ready yet the frame is dropped.
audio.setOnFrame((opusBytes) => {
  ...
  const chain = (activeCall._sendChain || Promise.resolve())
    .then(() => session.encrypt(opusBytes))
    .then((ct) => {
      if (!activeCall || !relay || !relay.connected) return;
      relay.sendAudio(ct);
    })
