...
function isEmscriptenMinified(moduleName, funcName) {
 ...
 */
export async function instantiateWasm(wasmBytes, exportMap) {
    let module;
    try {
        module = await WebAssembly.compile(wasmBytes);
    }
    catch (err) {
        throw new WasmLoadError(`Failed to compile WASM module: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    if (unknownImports.length > 0) {
        throw new WasmLoadError(`Unknown WASM imports detected: ${unknownImports.join(', ')}. ` +
    ...
    try {
        ...
        throw new WasmLoadError(`Failed to instantiate WASM instance: ${err instanceof Error ? err.message : String(err)}`);
    }
    let memory = null;
    ...
    if (missingExports.length > 0) {
        throw new WasmLoadError(`WASM module is missing required exports: ${missingExports.join(', ')}. ` +
...
//# sourceMappingURL=wasm-instantiate.js.map