...
import { selectWasmVariant } from './wasm-loader.js';
...
import { createStreamDenoiser as createStreamDenoiserImpl, } from './stream-denoiser.js';
import { loadEmbeddedWasm, loadEmbeddedWeights, loadEmbeddedExportMap, revokeProcessorBlobUrl, } from './embedded-loader.js';
const VALID_MODELS = ['tiny', 'base', 'small'];
const MODEL_SIZE_IDS = {
    tiny: 0,
    base: 1,
    small: 2,
};
const WEIGHT_FILENAMES = {
    tiny: 'fe_tiny_48k.bin',
    base: 'fe_base_48k.bin',
    small: 'fe_small_48k.bin',
};
function validateExportMap(data) {
    if (typeof data !== 'object' || data === null || Array.isArray(data)) {
        throw new WasmLoadError('Export map JSON must be a plain object');
    }
    const obj = data;
    for (const [key, value] of Object.entries(obj)) {
        if (typeof value !== 'string') {
            throw new WasmLoadError(`Export map entry "${key}" must be a string, got ${typeof value}`);
        }
    }
    return obj;
}
...
function getCacheKey(modelSize, simd, baseUrl) {
 ...
 */
export function clearCachedModel(modelSize, options) {
 ...
 */
export function loadModel(modelSize, options) {
 ...
 * ================================================================ */
async function loadModelImpl(modelSize, simdOption, baseUrlOption, signal) {
    const simdSupported = simdOption !== undefined ? simdOption : await detectSimd();
 ...
 * ================================================================ */
async function loadModelViaEmbed(modelSize, variant, signal) {
    let wasmBytes;
    let weightData;
    let exportMap;
    try {
        ...
        if (err instanceof ValidationError)
            throw err;
        throw new WasmLoadError(`Failed to load embedded model resources: ${err instanceof Error ? err.message : String(err)}`);
    }
    if (signal?.aborted) {
 ...
 * ================================================================ */
async function loadModelViaFetch(modelSize, variant, baseUrlOption, signal) {
 ...
 * ================================================================ */
function buildLoadedModel(modelSize, variant, wasmBytes, weightData, exportMap) {
    const modelSizeId = MODEL_SIZE_IDS[modelSize];
    const modelConfig = getModelConfig(modelSize);
    let cachedWasmPromise = null;
    ...
    return {
        ...
        async createStreamDenoiser(inputStream, opts) {
