...
const MODEL_SIZE_ID_MAP = {
    tiny: 0,
    base: 1,
    small: 2,
};
function resolveModelSizeId(model, explicitId) {
    if (explicitId !== undefined)
        return explicitId;
    const id = MODEL_SIZE_ID_MAP[model.size];
    if (id === undefined) {
        throw new ValidationError(`Unknown model size: "${model.size}". Valid values: tiny, base, small`);
    }
    return id;
}
function createDenoiserInstance(initialWasm, initialStatePtr, initialHopSize) {
    let currentState = 'ready';
    let bypassEnabled = false;
    let agcFlag = false;
    let hpfFlag = false;
    let switchCount = 0;
    let switchQueue = null;
    let wasm = initialWasm;
    let statePtr = initialStatePtr;
    let hopSize = initialHopSize;
    function getValidatedPointerOffset(ptr, label) {
        if (!ptr || ptr % 4 !== 0) {
            throw new ModelInitError(`Invalid WASM ${label} pointer: ${ptr} (must be non-zero and 4-byte aligned)`);
        }
        return ptr / 4;
    }
    ...
    function ensureReady(operation) {
    ...
    const denoiser = {
        ...
        processFrame(input) {
            ...
            if (input.length !== hopSize) {
                throw new ValidationError(`Expected ${hopSize} samples, got ${input.length}`);
            }
            for (let i = 0; i < input.length; i++) {
                if (!Number.isFinite(input[i])) {
                    throw new ValidationError(`Input contains non-finite value at index ${i}: ${input[i]}`);
                }
            }
            if (bypassEnabled) {
        ...
        get performance() {
            ...
            const p99Index = Math.min(Math.ceil(timingCount * 0.99) - 1, timingCount - 1);
        ...
        async switchModel(options) {
            ...
            const doSwitch = async () => {
                try {
                    ...
                    if (agcFlag)
                        newWasm._fe_set_agc(newStatePtr, 1);
                    if (hpfFlag)
                        newWasm._fe_set_hpf(newStatePtr, 1);
...
export async function isSupported() {
    ...
    const audioWorkletSupported = typeof globalThis !== 'undefined' &&
...
export { FastEnhancerError, WasmLoadError, ModelInitError, AudioContextError, WorkletError, ValidationError, DestroyedError, } from './errors.js';
