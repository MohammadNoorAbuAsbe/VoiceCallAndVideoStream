 ...
 * h_new = (1-z)*n + z*h
 */
export function gruStep(input, hidden, weights) {
    ...
    if (weights.W_z.length !== hiddenSize * inputSize ||
        ...
        throw new RangeError(`Input weight matrices must have length hiddenSize*inputSize (${hiddenSize}*${inputSize}=${hiddenSize * inputSize})`);
    }
    if (weights.U_z.length !== hiddenSize * hiddenSize ||
        ...
        throw new RangeError(`Hidden weight matrices must have length hiddenSize*hiddenSize (${hiddenSize}*${hiddenSize}=${hiddenSize * hiddenSize})`);
    }
    if (weights.b_z.length !== hiddenSize ||
        ...
        throw new RangeError(`Bias vectors must have length hiddenSize (${hiddenSize})`);
    }
