 ...
 */
export function conv1d(input, weight, bias, inLen, inChannels, outChannels, kernelSize, stride, padding) {
    if (stride <= 0)
        return new Float32Array(0);
    if (input.length < inChannels * inLen) {
        throw new RangeError(`input length (${input.length}) must be >= inChannels*inLen (${inChannels * inLen})`);
    }
    const expectedWeight = outChannels * inChannels * kernelSize;
    if (weight.length < expectedWeight) {
        throw new RangeError(`weight length (${weight.length}) must be >= outChannels*inChannels*kernelSize (${expectedWeight})`);
    }
    if (bias && bias.length < outChannels) {
        throw new RangeError(`bias length (${bias.length}) must be >= outChannels (${outChannels})`);
    }
...
export function stridedConv(input, weight, bias, inLen, inChannels, outChannels, kernelSize, stride) {
    if (stride <= 0)
        return new Float32Array(0);
    const outLen = Math.floor((inLen - kernelSize) / stride) + 1;
    if (outLen <= 0)
        return new Float32Array(0);
    return conv1d(input, weight, bias, inLen, inChannels, outChannels, kernelSize, stride, 0);
}
export function convTranspose1d(input, weight, bias, inLen, inChannels, outChannels, kernelSize, stride, padding) {
    if (stride <= 0)
        return new Float32Array(0);
    if (input.length < inChannels * inLen) {
        throw new RangeError(`input length (${input.length}) must be >= inChannels*inLen (${inChannels * inLen})`);
    ...
    if (weight.length < expectedWeight) {
        throw new RangeError(`weight length (${weight.length}) must be >= inChannels*outChannels*kernelSize (${expectedWeight})`);
    }
    if (bias && bias.length < outChannels) {
        throw new RangeError(`bias length (${bias.length}) must be >= outChannels (${outChannels})`);
    }
